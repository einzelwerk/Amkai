class AttachedFiles {
  constructor(container) {
    ...
    document.addEventListener('click', (event) => {
      if (event.target.closest('.attached-files__remove')) {
  ...
  renderFileList() {
    ...
    this.fileList.forEach((file, index) => {
      ...
      fileContainer.innerHTML = `<div class="attached-files__name">${file.name}</div> <button type="button" class="attached-files__remove" data-index="${index}">${svgClose}</button>`;
